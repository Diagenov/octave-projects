function spherical_potential(l)
  ...
  plot(X, Y_wv(:, 1), 'r', "linewidth", 1.5);
  plot(X, Y_wv(:, 2), 'g', "linewidth", 1.5);
  plot(X, Y_wv(:, 3), 'b', "linewidth", 1.5);
  hold off;




  dE = 10^(-6);         % приращение по энергии
  E = dE:dE:(1.1*B(3)); % промежуток сравнения
  E *= coef;            % эВ --> эрг
  ...
  title('Сравнение корней/нулей аналитического решения (красная линия) с численным решением (синие точки)', 'fontsize', 16);
  axis([0, E(end), y_min, y_max]);

  % сравнение численного решения с аналитикой через корни уравнения: jl(k*r0) = 0
  plot(E, y, 'r', "linewidth", 1.5);
  plot(B(1:3), [0, 0, 0], 'ob', "markersize", 10, "linewidth", 1.2);
  plot(B(1:3), [0, 0, 0], '+b', "markersize", 10, "linewidth", 1.2);
